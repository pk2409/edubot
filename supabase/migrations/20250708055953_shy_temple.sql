...
DO $$
BEGIN
  -- Enable RLS on question_papers if not already enabled
  IF NOT EXISTS (
  ...
  -- Enable RLS on grading_sessions if not already enabled
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
  ...
  -- Enable RLS on student_submissions if not already enabled
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
  ...
  -- Enable RLS on grading_analytics if not already enabled
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
  ...
  -- Enable RLS on ocr_jobs if not already enabled
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
