/*
  ...
  1. New Tables
    - `question_papers` - Store exam/quiz templates created by teachers
    - `grading_sessions` - Track grading sessions for batches of submissions
    - `student_submissions` - Store student answer images and grading results
    - `grading_analytics` - Store analytics data for grading sessions
    - `ocr_jobs` - Track OCR processing jobs for answer images

  2. Enhanced Users Table
    - Add grading preferences, statistics tracking

  3. Security
    - Enable RLS on all new tables
    - Add appropriate policies for teachers and students
    - Ensure data isolation between different teachers
...
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Teachers can manage own question papers'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'grading_sessions' 
    AND policyname = 'Teachers can manage own grading sessions'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Teachers can manage submissions in their sessions'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'grading_analytics' 
    AND policyname = 'Teachers can view analytics for their sessions'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'ocr_jobs' 
    AND policyname = 'Teachers can manage OCR jobs for their submissions'
  ) THEN
